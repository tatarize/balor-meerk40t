...
#         self.machine._wait_for_status_bits(query=ReadPort, wait_high=StopList)
#
...
#             if substitution_generator and n in substitutions:
#                 data = substitution_generator(data)
#             if direction:  # Read
...
#
#         if self.verbosity:
#             print("Ending lighting")
#         if not noend:
#             self.send_sequence(self.sequences.end)
#         self.lock.release()
#
#     # Sequence
#     # prefix: 0a6e-a89
...
#         if self.verbosity:
#             print("Waited %d cycles for laser to be ready." % count)
#
#         i = 0
#         while i < len(data):
#             packet = data[i : i + self.packet_size]
#             if self.verbosity:
#                 print("SENDING PACKET HERE %d:%d" % (i, i + self.packet_size))
#             i += self.packet_size
#             assert self.device.write(self.ep_homi, packet, 1000) == self.packet_size
#
#             if i == len(data):
#                 if self.verbosity:
#                     print("End data separator")
#                 self.send_sequence(self.sequences.mark_data_end)
#             else:
#                 if self.verbosity:
#                     print("Mark packet separator")
#                 self.send_sequence(self.sequences.mark_packet_separator)
#             count = self.wait_for_rv_bits(0x07, 0x20)
#             if self.verbosity:
#                 print("Waited %d cycles for laser to be ready for next packet." % count)
#
...
#         if self.verbosity:
#             print("Waited %d cycles for laser to be done." % count)
#         if self.verbosity:
#             print("Mark suffix")
