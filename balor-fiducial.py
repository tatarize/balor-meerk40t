...
parser.add_argument(
    "--travel-speed", help="Specify the traveling speed (mm/s)", default=200, type=float
)
parser.add_argument(
    "--cut-speed", help="Specify the cutting speed (mm/s)", default=100, type=float
)
parser.add_argument(
    "--size",
...
parser.add_argument(
    "-x", help="Specify the x position (mm) default 0", default=0, type=float
)
parser.add_argument(
    "-y", help="Specify the y position (mm) default 0", default=0, type=float
)
parser.add_argument(
    "--laser-power",
    "-p",
...
def render_fiducial(job, cal, args):
    ...
    xmin, ymin = cal.interpolate(
        args.x - args.size * 1e-3 / 2, args.y - args.size * 1e-3 / 2
    )
    xmax, ymax = cal.interpolate(
        args.x + args.size * 1e-3 / 2, args.y + args.size * 1e-3 / 2
    )
    # Cut the X line
    ...
    for _ in range(200):
        job.append(balor.MSBF.OpMarkEndDelay(0x100))
    job.append(balor.MSBF.OpAltTravel(0x0008))
