...
parser.add_argument(
    ...
    "--machine",
    help=(
        "specify which machine protocol to use. Valid machines: "
...
parser.add_argument(
    "--calfile", help="Provide a calibration file for patterns that use one."
)
...
parser.add_argument(
    "--xmin", help="Specify the minimum parameter value for x", default=0, type=int
)
parser.add_argument(
    "--xmax",
...
parser.add_argument(
    "--ymin", help="Specify the minimum parameter value for y", default=0, type=int
)
parser.add_argument(
    "--ymax",
...
parser.add_argument("--xsteps", help="How many steps in x?", default=0, type=int)
parser.add_argument("--ysteps", help="How many steps in y?", default=0, type=int)
parser.add_argument(
    "-c",
    "--cell",
    help="Gird or parameter cell size in native/galvo units",
    default=0x10000 // 0x40,
    type=int,
)
parser.add_argument(
    "operation",
...
class OldGridPattern(TestPattern):
    ...
    def render(self):
        ...
        if marking and ymin > cell_size and xmin > cell_size:
            ...
            self.job.append(
                balor.MSBF.OpJumpTo(ymin + fiducial_offset, xmin + fiducial_offset)
            )
            self.laser_power(True)
            self.job.append(
                balor.MSBF.OpMarkTo(
                    ymin + fiducial_offset, xmin + fiducial_offset + fiducial_size
                )
            )
            self.job.append(
                balor.MSBF.OpMarkTo(
                    ymin + fiducial_offset + fiducial_size,
                    xmin + fiducial_offset + fiducial_size,
                )
            )
            self.job.append(
                balor.MSBF.OpMarkTo(
                    ymin + fiducial_offset + fiducial_size, xmin + fiducial_offset
                )
            )
            self.job.append(
                balor.MSBF.OpMarkTo(ymin + fiducial_offset, xmin + fiducial_offset)
            )
            self.laser_power(False)
            # Make a triangle at max X
            self.job.append(
                balor.MSBF.OpJumpTo(ymin + fiducial_offset, xmax - fiducial_offset)
            )
            self.laser_power(True)
            self.job.append(
                balor.MSBF.OpMarkTo(
                    ymin + fiducial_offset, xmax - fiducial_offset + fiducial_size
                )
            )
            self.job.append(
                balor.MSBF.OpMarkTo(
            ...
            self.job.append(
                balor.MSBF.OpMarkTo(ymin + fiducial_offset, xmax - fiducial_offset)
            )
            self.laser_power(False)
...
class GridPattern(TestPattern):
    ...
    def render(self):
        ...
        for n in range(-4, 5):
            ...
            if not n:
                if marking:
                    self.laser_power(False)
                self.job.append(
                    balor.MSBF.OpJumpTo(0x8000 + n * cell_size, 0x8000 + cell_size)
                )
                if marking:
                    self.laser_power(True)
            else:
        ...
        for n in range(-4, 5):
            ...
            if not n:
                if marking:
                    self.laser_power(False)
                self.job.append(
                    balor.MSBF.OpJumpTo(0x8000 + cell_size, 0x8000 + n * cell_size)
                )
                if marking:
                    self.laser_power(True)
            else:
        ...
        self.job.append(balor.MSBF.OpJumpTo(0x8000, 0x8000))
        self.job.append(
            balor.MSBF.OpJumpTo(ymax - 5 * cell_size // 6, xmax - 5 * cell_size // 6)
        )
        if marking:
            self.laser_power(True)
    ...
    def render(self):
        ...
        for nx in range(-4, 4):
            for ny in range(-4, 4):
                self.job.append(
                    balor.MSBF.OpJumpTo(
                        *cal.interpolate(nx * 15.0 + 2.5, ny * 15.0 + 2.5)
                    )
                )
                if marking:
                    self.laser_power(True)
